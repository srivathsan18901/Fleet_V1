...
export class DashboardComponent implements AfterViewInit {
  ...
  roboToLocalize: number | null = null;
  ...
  get isTaskAssignedOrIsLocalize(): boolean {
    return (
      ...
      this.nodeGraphService.getLocalize()
    );
  }
  ...
  localNodePositions: any[] = [];
  localizationPos: { x: number; y: number; yaw: number } = {
    x: 0,
    y: 0,
    yaw: 0,
  };
  ...
  async getMapDetails() {
    ...
    const robotBaseHeight = 30; // This is the same base height used in plotRobo
    const robotHeight = robotBaseHeight ;
    this.rackSize = robotHeight * 1.2; 
    this.origin = {
      x: mapData.origin.x,
      y: mapData.origin.y,
      w: mapData.origin.w,
    ...
    this.localNodePositions = mapData.nodes.map((node: any) => {
      return {
        id: node.nodeId,
  ...
  plotRobo(
    ...
    if (ctx) {
      ...
      const shouldDrawErrCircle = Math.floor(Date.now() / 300) % 2 === 0;
      if (shouldDrawErrCircle) {
        ctx.beginPath();
        ctx.arc(0, errcircleRadius - height, errcircleRadius, 0, Math.PI * 2);
        ctx.fillStyle = errStateColor; // Set the error state circle color
        ctx.fill();
        ctx.closePath();
      }
